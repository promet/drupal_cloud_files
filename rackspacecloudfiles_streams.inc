...
class RackspaceCloudFilesStreamWrapper implements DrupalStreamWrapperInterface {
  ...
  public function unlink($uri) {
    ...
    if ($this->container->delete_object($this->getLocalPath($uri))) {
  ...
  public function url_stat($uri, $flags) {
    ...
    if(!$fileinfo) {
      ...
      try{
        if (empty($target)) {
          $filemime = "application/directory";
          $filesize = 1;
        }
        else {
          $this->object = $this->container->get_object($target);
